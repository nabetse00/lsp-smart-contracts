...
import {ERC725, ERC725X, ERC725Y} from "@erc725/smart-contracts/contracts/ERC725.sol";
...
import {NoExtensionFoundForFunctionSelector} from "../LSP17ContractExtension/LSP17Errors.sol";
    ...
    LSP20CallVerification
{
     ...
     */
    constructor(address newOwner) payable ERC725(newOwner) {
        if (msg.value != 0) {
     ...
     *
     * - If the data is equal or longer than 4 bytes, the ERC725Y storage is queried with the following data key:
    ...
    function execute(
        ...
        if (msg.sender == _owner) {
            return ERC725X._execute(operationType, target, value, data);
        }
        ...
        bytes memory result = ERC725X._execute(operationType, target, value, data);
    ...
    function execute(
        ...
        // If the caller is the owner perform execute directly
        if (msg.sender == _owner) {
            return ERC725X._execute(operationsType, targets, values, datas);
        }
        ...
        // Perform the execution
        bytes[] memory results = ERC725X._execute(operationsType, targets, values, datas);

        // if verifyAfter is true, Call {lsp20VerifyCallResult} on the owner
        if (verifyAfter) {
            LSP20CallVerification._verifyCallResult(_owner, abi.encode(results));
        }
    ...
    ) public virtual override(LSP14Ownable2Step, Ownable) {
        address currentOwner = owner();

        // If the caller is the owner perform transferOwnership directly
        if (msg.sender == currentOwner) {
            // set the pending owner
            LSP14Ownable2Step._setPendingOwner(_pendingOwner);
     ...
     */
    function renounceOwnership() public virtual override(LSP14Ownable2Step, Ownable) {
        address _owner = owner();
    ...
    function supportsInterface(
        bytes4 interfaceId
    ) public view virtual override(ERC725, LSP17Extendable) returns (bool) {
        return
     ...
     *
     * - If the owner is an EOA, recovers an address from the hash and the signature provided:
     *
     *      - Returns the magicValue if the address recovered is the same as the owner, indicating
     *        that it was a valid signature.
     *
     *      - If the address is different, it returns the fail value indicating that the signature is not valid.
     *
     * - If the owner is a smart contract, it forwards the call of {isValidSignature} to the owner contract:
     *
     *      - If the contract fails or returns the fail value, the {isValidSignature} on the account returns the
     *        fail value, indicating that the signature is not valid.
     *
     *      - If the {isValidSignature} on the owner returned the magicValue, the {isValidSignature} on the account
     *        returns the magicValue, indicating that it's a valid signature.
     *
    ...
    function _getExtension(
        ...
        address extension = address(bytes20(ERC725Y._getData(mappedExtensionDataKey)));

        return extension;
    }

    /**
     * @dev This function overrides the {ERC725Y} internal {_setData} function to optimize gas usage by emitting the
    ...
    function _setData(bytes32 dataKey, bytes memory dataValue) internal virtual override {
        ERC725Y._store[dataKey] = dataValue;
        emit DataChanged(
            dataKey,
